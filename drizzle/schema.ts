import { pgTable, varchar, text, timestamp, index, foreignKey, unique, date, boolean, numeric, integer } from "drizzle-orm/pg-core"
...
export const empleado = pgTable("empleado", {
	...
	documentoIdentidad: varchar("documento_identidad", { length: 30 }).notNull(),
	...
	index("idx_empleado_documento").using("btree", table.documentoIdentidad.asc().nullsLast().op("text_ops")),
	foreignKey({
...
export const bodega = pgTable("bodega", {
...
export const producto = pgTable("producto", {
	...
	index("idx_producto_codigo").using("btree", table.codigo.asc().nullsLast().op("text_ops")),
	index("idx_producto_nombre").using("btree", table.nombre.asc().nullsLast().op("text_ops")),
...
export const stock = pgTable("stock", {
	...
	index("idx_stock_producto").using("btree", table.idProducto.asc().nullsLast().op("text_ops")),
	foreignKey({
	...
	unique("stock_id_producto_id_bodega_key").on(table.idProducto, table.idBodega),
]);
...
export const compra = pgTable("compra", {
	...
	index("idx_compra_fecha").using("btree", table.fecha.asc().nullsLast().op("date_ops")),
	foreignKey({
...
export const compraDetalle = pgTable("compra_detalle", {
	...
	index("idx_compra_detalle_producto").using("btree", table.idProducto.asc().nullsLast().op("text_ops")),
	foreignKey({
...
export const movimientoBodega = pgTable("movimiento_bodega", {
	idMovimiento: varchar("id_movimiento", { length: 50 }).primaryKey().notNull(),
	idProducto: varchar("id_producto", { length: 50 }),
...
export const cliente = pgTable("cliente", {
	...
	index("idx_cliente_documento").using("btree", table.documentoIdentidad.asc().nullsLast().op("text_ops")),
	index("idx_cliente_email").using("btree", table.email.asc().nullsLast().op("text_ops")),
...
export const factura = pgTable("factura", {
	...
	impuestoTotal: numeric("impuesto_total", { precision: 10, scale:  2 }).default('0'),
	descuentoTotal: numeric("descuento_total", { precision: 10, scale:  2 }).default('0'),
	total: numeric({ precision: 10, scale:  2 }).notNull(),
	estado: varchar({ length: 20 }),
	...
	index("idx_factura_cliente").using("btree", table.idCliente.asc().nullsLast().op("text_ops")),
	index("idx_factura_fecha").using("btree", table.fecha.asc().nullsLast().op("timestamp_ops")),
	index("idx_factura_sucursal").using("btree", table.idSucursal.asc().nullsLast().op("text_ops")),
	foreignKey({
...
export const facturaDetalle = pgTable("factura_detalle", {
	...
	index("idx_factura_detalle_producto").using("btree", table.idProducto.asc().nullsLast().op("text_ops")),
	foreignKey({
...
export const devolucion = pgTable("devolucion", {
	idDevolucion: varchar("id_devolucion", { length: 50 }).primaryKey().notNull(),
	idFactura: varchar("id_factura", { length: 50 }),
	idEmpleado: varchar("id_empleado", { length: 50 }),
	fecha: timestamp({ mode: 'string' }).default(sql`CURRENT_TIMESTAMP`),
	total: numeric({ precision: 10, scale:  2 }).notNull(),
...
export const devolucionDetalle = pgTable("devolucion_detalle", {
	idDetalleDev: varchar("id_detalle_dev", { length: 50 }).primaryKey().notNull(),
...
export const devolucionProveedor = pgTable("devolucion_proveedor", {
	idDevolucionProv: varchar("id_devolucion_prov", { length: 50 }).primaryKey().notNull(),
	idCompra: varchar("id_compra", { length: 50 }),
	idEmpleado: varchar("id_empleado", { length: 50 }),
	fecha: timestamp({ mode: 'string' }).default(sql`CURRENT_TIMESTAMP`),
	total: numeric({ precision: 10, scale:  2 }).notNull(),
...
export const devolucionProveedorDetalle = pgTable("devolucion_proveedor_detalle", {
	idDetalleDevProv: varchar("id_detalle_dev_prov", { length: 50 }).primaryKey().notNull(),
...
export const promocionProducto = pgTable("promocion_producto", {
	idPromocionProducto: varchar("id_promocion_producto", { length: 50 }).primaryKey().notNull(),
	...
	unique("promocion_producto_id_promocion_id_producto_key").on(table.idPromocion, table.idProducto),
]);
...
export const transaccionCaja = pgTable("transaccion_caja", {
	idTransaccion: varchar("id_transaccion", { length: 50 }).primaryKey().notNull(),
...
export const rolPermiso = pgTable("rol_permiso", {
	idRolPermiso: varchar("id_rol_permiso", { length: 50 }).primaryKey().notNull(),
	rol: varchar({ length: 50 }),
...
export const auditoria = pgTable("auditoria", {
	...
	index("idx_auditoria_fecha").using("btree", table.fecha.asc().nullsLast().op("timestamp_ops")),
	foreignKey({
